...
class External_Attention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None):
        ...
        query = query / math.sqrt(model_dim)

        scores = torch.matmul(query, key.transpose(1, 2))   # [batch, q_len, k_len]

        if mask is not None:
            scores = scores.masked_fill(mask, -1e18)

        attn = self.softmax(scores)  # [batch, q_len, k_len]

        context = torch.matmul(attn, value)   # [batch, q_len, size]
...
class Hierarchical_Attention(nn.Module):
    ...
    def __init__(self, model_dim):
        ...
        self.model_dim = model_dim
        self.token_attention = External_Attention(model_dim)
        self.sent_attention = External_Attention(model_dim)

        self.final_linear = nn.Linear(model_dim, model_dim)

    def forward(self, token_vec, sent_vec, query, token_pad_mask, sent_pad_mask):
        _, token_attn = self.token_attention(token_vec, token_vec, query, token_pad_mask) # [batch, q_len, k_len]
        _, sent_attn = self.sent_attention(sent_vec, sent_vec, query, sent_pad_mask) # [batch, q_len, k_len]

        attn = self.masked_attention(token_attn, sent_attn, token_pad_mask)

        context = torch.matmul(attn, token_vec)   # [batch, q_len, size]
    ...
    def masked_attention(self, attn, sent_attn, token_pad_mask):
        d1, d2, d3 = sent_attn.size()
        ret_attn = attn

        sent_attn = sent_attn.unsqueeze(-1)
        attn = attn.view(d1, d2, d3, -1)
        attn = attn * sent_attn
        attn_norm = attn.view(d1, d2, -1)
        attn_dist = attn_norm / torch.sum(attn_norm, 2).unsqueeze(-1)

        attn_dist += 1e-8
        attn_dist *= (~token_pad_mask) # apply mask
        masked_sums = torch.sum(attn_dist, 2) # shape (batch_size)
        attn_dist /= masked_sums.view(d1, d2, 1) # re-normalize
...
class Multihead_Attention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None,
        ...
        if layer_cache is not None:
            ...
            # decoder context attention in the inference stage
                ...
                if layer_cache is not None:
                    ...
                    key = shape(key)    # [batch, head, k_len, head_dim]
                    value = shape(value)    # [batch, head, v_len, head_dim]
            # decoder other attention in the inference stage
            else:
                query = self.linear_query(query)
                # decoding after the first step, with layer cache
                if layer_cache is not None:
                    # device name for correct saving
                    if layer_cache[type + "_keys_" + device] is None:
                        key, value = self.linear_keys(key),\
                            self.linear_values(value)   # [batch, len, size]
                        key = shape(key)    # [batch, head, k_len, head_dim]
                        value = shape(value)    # [batch, head, v_len, head_dim]
                    else:
                        key, value = layer_cache[type + "_keys_" + device],\
                            layer_cache[type + "_values_" + device]
                    layer_cache[type + "_keys_" + device] = key
                    layer_cache[type + "_values_" + device] = value
                else:
                    key, value = self.linear_keys(key),\
                        self.linear_values(value)   # [batch, len, size]
                    key = shape(key)    # [batch, head, k_len, head_dim]
        ...
        if type in ["context","document"] and Bernoulli:
